...
class BayesianAlphaOptimizer:
    def __init__(self, model, calibration_data):
        ...
        # Pre-compute discrete alpha values
        self.granularity = 0.1
        self.discrete_alphas = self._generate_discrete_alphas()
        print(f"Discrete alpha values: {self.discrete_alphas}")

    
    def _generate_discrete_alphas(self):
        """Generate discrete alpha values based on granularity"""
        num_points = int(1.0 / self.granularity) + 1
        alphas = [round(i * self.granularity, 3) for i in range(num_points)]
        
        # Ensure 0.0 and 1.0 are included
        if alphas[-1] != 1.0:
            alphas.append(1.0)
    ...
    def optimize_layer_alpha(self, layer_name, n_calls=20):
        ...
        # Method 1: Using Integer space (推荐)
        num_points = len(self.discrete_alphas)
        search_space = [Integer(0, num_points - 1, name='alpha_idx')]
        
        # 定义目标函数
        @use_named_args(search_space)
        def objective(**params):
            alpha_idx = params['alpha_idx']
            alpha = self.discrete_alphas[alpha_idx]
        ...
        result = gp_minimize(
            ...
            n_calls=n_calls,
            n_initial_points=min(10, num_points),
            random_state=42,
            verbose=False
        )
        
        optimal_idx = result.x[0]
        optimal_alpha = self.discrete_alphas[optimal_idx]
