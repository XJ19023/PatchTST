...
from mycode.globalVar import (get_iterationCounter,
...
def pseudo_quantize_tensor( w, 
    ...
    if inplace:
        ...
        if spark_quant_en:
            ...
            w_int = tensor_01 + tensor_10 + tensor_left
...
class quantLinear(torch.nn.Linear):
    ...
    def enable_quant(self, quant_en=True, clamp_en=True, search_en=False, spark_en=False):
    ...
    def forward(self, x):
        if self.search:
            ...
            for percentile in [(1,   1), (1,   0.9), (1,   0.8), (1,   0.7), (1,   0.6), (1,   0.5), (1,   0.4), (1,   0.3), (1,   0.2), (1,   0.1),
                ...
                q_x, mse_int5_th, mse_int6_th = self.act_quant(x, clam_quant_en=self.clamp_en, percentile=percentile, search=True, spark_quant_en=self.spark_en)
                ...
                if mse < mse_th:
                    break
            with open('log/mse.txt', 'a') as f:
                f.writelines(f'>> mse: {mse:.6f}, percentile: {percentile}, {self.name}\n')
        ...
        q_x = self.act_quant(x, clam_quant_en=self.clamp_en, mse_int5_th=self.mse_int5_th, mse_int6_th=self.mse_int6_th, profile=True, spark_quant_en=self.spark_en)
        y = torch.functional.F.linear(q_x, self.weight, self.bias)
